...
function fchat.FixInvalidFont()
	local ok = pcall(draw.SimpleText, "shit", fchat.font:GetString(), 0, 0, color_transparent, 1, 1)

	if (!ok) then
		fchat.font:SetString("fchat.theme.text")

		Msg("Reverting fchat font because it is invalid!\n")
...
function fchat.ScaleSize(amount, x)
...
hook.Add("PostGamemodeLoaded", "fchat.AddGamemodeFunctions", function()
	...
	function GAMEMODE:FchatCanUseExpressions(player)
		return true
	end
	...
	function GAMEMODE:FchatCanUseExpression(player, expression, unique)
		if (IsValid(player)) then
			local data = fchat.restrictions[unique]
...
hook.Add("InitPostEntity", "fchat.CreateChatbox", function()
	fchat.theme.DeriveThemes()

	local theme = fchat.theme.GetCurrent()

	if (!theme) then
		Msg("Invalid fchat theme! Reverting back to default.\n")

		fchat.themeConfig:SetString("default")
	end

	local panel = fchat.theme.GetValue("panel")

	if (!ValidPanel(panel)) then
		fchat.theme.Call("Initialize")

		fchat.themeConfig:OnChange(fchat.themeConfig:GetString())

		fchat.FixInvalidFont()

		fchat.fontHeight = draw.GetFontHeight(fchat.font:GetString())

		net.Start("fchat.plload")
		net.SendToServer()
	end
end)
...
hook.Add("DrawOverlay", "fchat.DrawCopiedNotify", function()
	for i = 1, #copiedNotify do
		local data = copiedNotify[i]

		if (data) then
			draw.SimpleText("Copied text!", "fchat.theme.text.shadow", data.x, data.y, color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
			draw.SimpleText("Copied text!", "fchat.theme.text", data.x +1, data.y +1, color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
			draw.SimpleText("Copied text!", "fchat.theme.text", data.x, data.y, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
...
hook.Add("HUDShouldDraw", "fchat.HUDShouldDraw", function(id)
	local panel = fchat.theme.GetValue("panel")
...
hook.Add("PlayerBindPress", "fchat.PlayerBindPress", function(player, bind, pressed)
	if (bind == "messagemode" and pressed) then
		...
		if (ValidPanel(panel)) then
			...
			fchat.theme.Call("OnToggle", true)
...
hook.Add("player_disconnect", "fchat.DisconnectMessage", function(data)
	local filtered = fchat.filterJoinDisconnect:GetBool()
...
net.Receive("fchat.plcnt", function(bits)
...
hook.Add("ChatText", "fchat.ChatText", function(index, name, text, filter)
	if (tonumber(index) == 0) then
		...
		elseif (filter == "none") then
			if (name == "Console") then
				...
				if (ValidPanel(panel)) then
					fchat.theme.Call("ParseText", nil, color_white, text)
				end
			end
...
if (!fchat.chatAddText) then fchat.chatAddText = chat.AddText end

function chat.AddText(...)
	local panel = fchat.theme.GetValue("panel")

	if (ValidPanel(panel)) then
		fchat.theme.Call("ParseText", nil, ...)
	end
...
fchat.fontCache = fchat.fontCache or {}

function fchat.BuildFontCache(text, font)
	local text = tostring(text)
	text = __clean(text, " ")
	text = text:gsub("\t", "    ") -- a shitty hack for tabs

	local font = font or fchat.font:GetString()
	local lowered = string.lower(font)
	local len = string.utf8len(text)

	if (!fchat.fontCache[lowered]) then fchat.fontCache[lowered] = {} end
	...
	for i = 1, len do
		...
		if (!fchat.fontCache[lowered][character]) then
			local width = surface.GetTextSize(character)

			fchat.fontCache[lowered][character] = width
		end
	end

	width = surface.GetTextSize("�")

	fchat.fontCache[lowered]["�"] = width
...
function fchat.NewBasePanel()
	local spacing = fchat.theme.GetValue("messageSpacing") or 2

	local panel = vgui.Create("Panel")
	panel:SetTall(fchat.fontHeight)
	panel:Dock(TOP)
	panel:DockMargin(0, 0, 0, spacing)

	panel.fade = CurTime() +fchat.fadetime:GetInt()
	panel.selection = {}
	...
	function panel:Think()
		local chatPanel = fchat.theme.GetValue("panel")
									...
									if (x >= x2 and x <= x2 +child:GetWide()) then
										if (child:GetClassName() == "Label") then
											...
											while start <= len do
												...
												local width = fchat.fontCache[font][character]
	...
	function panel:PaintOver(w, h)
		if (self.selection and self.selection.width and self.selection.x) then
			local color = fchat.theme.GetValue("color").selection
						...
						for i = 1, #sortedChildren do
							...
							if ((x >= x2 or x2 >= x) and (width <= x2 +wide or width >= x2 +wide)) then
								if (child:GetClassName() == "Label") then
									...
									while start <= len do
										local character = string.utf8sub(text, start, start)
										local characterWidth = fchat.fontCache[font][character]
	...
	if (fchat.messageFadeIn:GetBool()) then
		panel:SetAlpha(0)
...
local function b(parent, noWrap, font)
	local fontHeight = font and draw.GetFontHeight(font) or fchat.fontHeight
...
function fchat.ParseExpressionPreview(parent, font, noWrap)
...
function fchat.GenericLabel()
	local font = fchat.font:GetString()
	...
	function label:Paint(w, h)
		local shadow, text, font, extraShadow, pcall = shadow, self:GetText(), self:GetFont(), fchat.extraShadow:GetBool(), pcall
...
function fchat.LabelAndCheckbox(parent, name)
	local base = parent:Add("Panel")
	base:SetTall(15)
	base:Dock(TOP)
...
function fchat.LabelAndNumSlider(parent, name)
	local base = parent:Add("Panel")
	base:SetTall(20)
	base:Dock(TOP)
	base:DockMargin(0, 0, 0, 10)
...
function fchat.LabelAndOption(parent, name)
...
net.Receive("fchat.nwpm", function(bits)
	local key = net.ReadUInt(8)
	local panel = fchat.theme.GetValue("panel").chatrooms
...
net.Receive("fchat.rxpm", function(bits)
	local key = net.ReadUInt(8)
	...
	if (player != LocalPlayer()) then
		fchat.theme.Call("ParseText", nil, color_red, "(PM) ", player, color_white, ": ", text)
	end

	-- Add it to the private chat.
	fchat.theme.Call("ParseText", panel:GetListByKey(key), player, color_white, ": ", text)
end)
...
net.Receive("fchat.gtplpm", function(bits)
	local key = net.ReadUInt(8)
...
net.Receive("fchat.nkickpm", function(bits)
	local key = net.ReadUInt(8)
	local target = net.ReadEntity()
	local noLocal = util.tobool(net.ReadBit())
	local panel = fchat.theme.GetValue("panel").chatrooms:GetChatroom(key)
...
net.Receive("fchat.sinvpm", function(bits)
	local key = net.ReadUInt(8)
	local player = net.ReadEntity()

	local accept = fchat.GenericLabel()
...
net.Receive("fchat.msg", function(bits)
	local text = net.ReadString()

	chat.AddText(color_red, "[atlashchat] ", color_white, text)
end)

function fchat.Notify(text)
	chat.AddText(color_red, "[atlashchat] ", color_white, text)
end
...
net.Receive("fchat.clrcfg", function(bits)
	file.Delete("fchat_config.cfg", "DATA")

	fchat.config.ResetValues()

	local config = fchat.theme.GetValue("config")
...
net.Receive("fchat.crtrnkgt", function(bits)
	...
	if (LocalPlayer():IsSuperAdmin()) then
		local rankMenu = fchat.theme.GetValue("rankMenu")
...
net.Receive("fchat.rstcs", function(bits)
	...
	if (remove and fchat.restrictions[expression]) then
		fchat.restrictions[expression][usergroup] = nil
	end

	local restrictionPanel = fchat.theme.GetValue("restrictionPanel")
...
net.Receive("fchat.chatText", function(bits)
	local text = net.ReadString()
	local player = net.ReadEntity()
