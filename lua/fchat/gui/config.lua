surface.CreateFont("fchat.config", {font = "Open Sans", size = 18, weight = 400})
surface.CreateFont("fchat.config.category", {font = "Open Sans", size = 18, weight = 400})
...
function panel:AddButton(name, admin, callback)
	...
	function button:Paint(w, h)
		...
		draw.SimpleText(self.name, "fchat.config", w /2, h /2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
...
function panel:AddItem(left, right)
	...
	function panel:Paint(w, h)
		...
		draw.SimpleText(self.name, "fchat.config", 12, h /2, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
...
function panel:AddCategory(name, value)
	...
	function panel:Paint(w, h)
		...
		draw.SimpleText(self.name, "fchat.config.category", 12, h /2, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end
	
	self.list:AddItem(panel)
end

vgui.Register("fchat.config", panel, "fchat.frame")
