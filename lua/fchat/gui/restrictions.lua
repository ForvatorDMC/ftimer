surface.CreateFont("fchat.restrictions", {font = "Open Sans", size = 14, weight = 400})
...
function panel:Populate()
	...
	for i = 1, #expressions do
		...
		local base = vgui.Create("fchat.restrictions.row")
		base:SetTall(40)
		base:Dock(TOP)
		base:SetText(text)
		base:SetCursor("hand")
...
vgui.Register("fchat.restrictions", panel, "fchat.frame")
...
function panel:Init()
	...
	self.button:SetImage("fchat/plus.png")
...
function panel:AddUsergroup(usergroup)
	local width, height = util.GetTextSize("fchat.restrictions", usergroup)
	...
	function panel:Paint(w, h)
		...
		draw.SimpleText(self.usergroup,"fchat.restrictions", w /2, h /2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
...
function panel:GetBase()
	local width, height = util.GetTextSize("fchat.restrictions", "M")
...
function panel:Rebuild()
	...
	local data = fchat.restrictions[self.expression]
...
function panel:Paint(w, h)
	...
	draw.SimpleText(self.text, "fchat.config", 12, 40 /2, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	
end

vgui.Register("fchat.restrictions.row", panel, "Panel")
