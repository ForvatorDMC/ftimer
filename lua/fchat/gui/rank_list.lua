surface.CreateFont("fchat.ranks.column", {font = "Open Sans", size = 15, weight = 400})
...
function panel:Init()
	...
	function self.button:Paint(w, h)
		...
		draw.SimpleText("SETUP NEW RANK", "fchat.config", w /2, h /2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
	
	function self.button:OnMousePressed()
		Derma_StringRequest("Add Usergroup", "Enter the name/unique of the usergroup", "", function(text) if (text != "") then net.Start("fchat.crtrnk") net.WriteString(text) net.SendToServer() end end, function(text) end, "Accept")
	end
...
function panel:AddColumn(name, width)
	local column = self.columnBase:Add("fchat.ranks.column")
...
function panel:UpdateLine(unique, image, tag)
	...
	for k, child in pairs(children) do
		if (child.unique == unique) then
			...
			fchat.ParseExpressionPreview(child.tag, "DermaDefault", true)
		end
	end
end
...
local deleteImage = Material("fchat/cross.png", "smooth")
...
function panel:Populate()
	...
	for unique, data in pairs(fchat.ranks) do
		...
		function usergroup.button:OnMousePressed()
			Derma_Query("Are you sure you want to delete this rank?", "Delete Rank", "Accept", function() net.Start("fchat.rmvrnk") net.WriteString(usergroup.text) net.SendToServer() end, "Cancel", function() end)
		end
		...
		function image:OnMousePressed()
			local menu = DermaMenu()
				menu:AddOption("Change Image", function()
					...
					local base = vgui.Create("fchat.frame")
					base:SetSize(266, 384)
					base:SetPos(x, y)
					base:SetTitle("SELECT ICON")
					base:MakePopup()
					
					local iconList = base:Add("fchat.chat.list")
					iconList:Dock(FILL)
		...
		timer.Simple(0.1, function() fchat.ParseExpressionPreview(tag, "DermaDefault", true) end)
		
		self:AddLine(unique, usergroup, image, tag)
	end
end

vgui.Register("fchat.ranks", panel, "fchat.frame")
...
function panel:Paint(w, h)
	draw.SimpleText(self.text, "fchat.ranks.column", self.align_x == TEXT_ALIGN_CENTER and w /2  or self.align_x == TEXT_ALIGN_RIGHT and w -3 or 4, 0, color_column, self.align_x, TEXT_ALIGN_TOP)
end

vgui.Register("fchat.ranks.column", panel, "Panel")
