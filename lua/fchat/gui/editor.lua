surface.CreateFont("fchat.editor.label", {font = "Open Sans Semibold", size = 18, weight = 400})
surface.CreateFont("fchat.editor.button", {font = "Open Sans", size = 15, weight = 400})
...
function panel:Init()
	...
	function self.finish:Paint(w, h)
		...
		draw.SimpleText("Complete", "fchat.editor.button", w /2, h /2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
	...
	function self.cancel:Paint(w, h)
		...
		draw.SimpleText("Cancel", "fchat.editor.button", w /2, h /2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
	...
	label:SetFont("fchat.editor.label")
	...
	function base:Think()
		if (self.nextUpdate and self.nextUpdate <= CurTime()) then
			fchat.ParseExpressionPreview(base)
...
vgui.Register("fchat.editor", panel, "fchat.form")
