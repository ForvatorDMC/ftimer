surface.CreateFont("fchat.slider.text", {font = "DeJaVu Sans", size = 12, weight = 400})
...
function panel:PaintOver(w, h)
	if (self.Hovered or self.Knob.Hovered or self.Knob.Depressed) then
		surface.DisableClipping(true)
			draw.SimpleText(self:GetValue(), "fchat.slider.text", self.Knob.x -1 +self.Knob:GetWide() /2 +1, self.Knob.y -8, color_black, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			draw.SimpleText(self:GetValue(), "fchat.slider.text", self.Knob.x -1 +self.Knob:GetWide() /2, self.Knob.y -9, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		surface.DisableClipping(false)
	end
end

vgui.Register("fchat.slider", panel, "DSlider")
