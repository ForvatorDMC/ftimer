...
function panel:Init()
	...
	self.scrollbar = self:Add("fchat.scrollbar")
...
function panel:AddChatroom(name, key, newList)
	local panel = self.canvas:Add("fchat.chatroom")
	panel:SetName(name)
	panel:SetKey(key)
	...
	if (newList) then
		...
		local list = chatPanel:Add("fchat.chat.list")
		list:Dock(FILL)
...
vgui.Register("fchat.chatroom.container", panel, "Panel")
...
function panel:ThrowIntoBlackHole()
	...
	local chatroom, userList = fchat.theme.GetValue("mainChat"), fchat.theme.GetValue("userListBase")
...
function panel:AddPlayer(player, creator)
	...
	if (!exists or creator) then
		if (!ValidPanel(self.label)) then
			...
			self.label:SetFont("fchat.theme.list.name")
...
function panel:RemovePlayer(player, left)
	...
	-- Some other player was kicked or left the chatroom
		...
		if (left) then
			fchat.theme.Call("ParseText", list, ":information: " .. nick .. " has left the chatroom.")
		else
			fchat.theme.Call("ParseText", list, color_red, ":exclamation: " .. nick .. " was kicked from the chatroom!")
		end
...
function panel:PerformLayout()
	if (ValidPanel(self.label)) then
		...
		local width = util.GetTextSize("fchat.theme.list.name", text)
...
function panel:OnMousePressed(code)
	...
	if (code == MOUSE_LEFT) then
		...
		fchat.theme.Set("current_chatroom", self)
...
function panel:Paint(w, h)
	fchat.theme.Call("PaintChatroom", self, w, h)

	return true
end

vgui.Register("fchat.chatroom", panel, "Panel")
...
function panel:Init()
	...
	self.grip = self:Add("fchat.scrollbar.grip")
...
vgui.Register("fchat.scrollbar", panel, "Panel")
...
vgui.Register("fchat.scrollbar.grip", panel, "Panel")
