surface.CreateFont("fchat.mysql", {font = "Open Sans", size = 18, weight = 400})
...
function panel:Init()
	...
	function self.statusPanel:Paint(w, h)
		local _, height = util.GetTextSize("fchat.mysql", self.status)
		
		draw.DrawText(self.status, "fchat.mysql", w /2, h /2 -height /2, self.color, TEXT_ALIGN_CENTER)
	...
	self:AddItem("Database ( Default \"fchat\" )", self.database)
...
function panel:SetData(status, message, address, port, username, database)
	...
	local _, height = util.GetTextSize("fchat.mysql", self.statusPanel.status)
...
vgui.Register("fchat.mysql", panel, "fchat.config")
...
net.Receive("fchat.myin", function(bits)
	...
	local panel = fchat.theme.GetValue("mysqlPanel")
