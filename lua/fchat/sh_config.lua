...
local saved = file.Read("fchat_config.txt", "DATA")
local nextSave = nil
...
function fchat.config.New(text, name, default, save, disableCommand, server, force)
	local object
	local exists = false
	...
	if (!disableCommand) then
		concommand.Add("fchat_" .. name, function(player, command, arguments)
			local value = arguments[1]
			
			if (value) then
				local config = stored[string.sub(command, string.len("fchat") +2)]
...
function fchat.config.GetStored()
	return stored
end
...
function fchat.config.Get(name)
	return stored[name]
end
...
function fchat.config.ResetValues()
	for i = 1, #objects do
...
hook.Add("Tick", "fchat.config.Tick", function()
	if (nextSave and nextSave <= CurTime()) then
		file.Write("fchat_config.txt", von.serialize(saved), "DATA")
...
if (CLIENT) then
	...
	net.Receive("fchat.sndcfg", function(bits)
...
if (SERVER) then
	...
	util.AddNetworkString("fchat.gtcfg")
	util.AddNetworkString("fchat.sndcfg")
	
	net.Receive("fchat.gtcfg", function(bits, player)
	...
	function fchat.config.SyncVariables(player)
...
fchat.enableAvatars = fchat.config.New("Enable avatars", "avatars", true, true, true, true)
fchat.enableWyoziteTags = fchat.config.New("Enable wyozite tags", "wyozite_tags", true, true, true, true)
fchat.enableRankIcons = fchat.config.New("Enable usergroup icon", "rank_icons", true, true, true, true)
fchat.enableRankTitle = fchat.config.New("Enable usergroup title", "rank_title", true, true, true, true)
fchat.darkrpChat = fchat.config.New("Show titles & icons everywhere (DARKRP)", "tidarkrp", false, true, true, true)
