...
local plusImage = Material("fchat/plus.png")
...
function panel:Init()
	...
	self.hostName:SetFont("fchat.theme.default.title")
	...
	function self.iconHolder:Paint(w, h)
		fchat.theme.Call("PaintIconHolder", self, w, h)
	...
	self.button = self:Add("fchat.chat.button")
	self.button:SetSize(16, 16)
	self.button:SetZPos(100)
	
	function self.button.DoClick()
		net.Start("fchat.stpm")
		net.SendToServer()
	end
...
function panel:ChangeList(key, list)
	...
	if (key) then
		if (!ValidPanel(self.userList)) then
			self.userList = self:AddIcon("fchat/users.png", function() fchat.theme.Call("ToggleUserList") end, "Chat Userlist")
		end
		
		local userListBase = fchat.theme.GetValue("userListBase")
		
		if (IsValid(userListBase)) then
			local chatroom = fchat.theme.GetValue("current_chatroom")
...
function panel:Think()
	...
	if (self.Dragging[1] != 0) then
		...
		fchat.chat_x:SetInt(x)
		fchat.chat_y:SetInt(y)
	end
	...
	if (self.Sizing) then
		...
		fchat.chat_w:SetInt(x)
		fchat.chat_h:SetInt(y)
...
function panel:Paint(w, h)
	fchat.theme.Call("PaintPanel", w, h)
end

vgui.Register("fchat.chat", panel, "EditablePanel")
...
function panel:Paint(w, h)
	fchat.theme.Call("PaintButton", self, w, h)
	
	return true
end

vgui.Register("fchat.chat.button", panel, "DButton")
...
function panel:Init()
	...
	function self.VBar:Paint(w, h)
		fchat.theme.Call("PaintScrollbar", self, w, h)
	end
	
	function self.VBar.btnGrip:Paint(w, h)
		fchat.theme.Call("PaintScrollbarGrip", self, w, h)
	end
	
	function self.VBar.btnUp:Paint(w, h)
		fchat.theme.Call("PaintScrollbarUpButton", self, w, h)
	end
	
	function self.VBar.btnDown:Paint(w, h)
		fchat.theme.Call("PaintScrollbarDownButton", self, w, h)
	end
...
function panel:AddItem(panel)
	...
	if (self.deleteHistory) then
		local current, maxHistory = #self.history, fchat.maxHistory:GetInt()
...
function panel:Paint(w, h)
	...
	fchat.theme.Call("PaintList", panel, w, h, self)
...
vgui.Register("fchat.chat.list", panel, "DScrollPanel")
...
function panel:OnKeyCodeTyped(code)
	...
	elseif (code == KEY_ENTER) then
		...
		if (value != "") then
			if (self.key) then
				net.Start("fchat.txpm")
					net.WriteUInt(self.key, 8)
					net.WriteString(value)
				net.SendToServer()
			else
				net.Start("fchat.chat")
					net.WriteString(value)
...
function panel:Paint(w, h)
	fchat.theme.Call("PaintTextEntry", w, h)
end

vgui.Register("fchat.chat.entry", panel, "DTextEntry")
...
function panel:SetPrefix(prefix)
	...
	local w = util.GetTextSize("fchat.theme.prefix", prefix)
...
function panel:Paint(w, h)
	fchat.theme.Call("PaintPrefix", w, h)
end

vgui.Register("fchat.chat.prefix", panel, "Panel")
