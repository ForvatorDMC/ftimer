...
surface.CreateFont("fchat.theme.default.title", {font = "Open Sans", size = 19, weight = 400})
surface.CreateFont("fchat.theme.userlist", 		{font = "Roboto Lt", size = 18, weight = 400})
surface.CreateFont("fchat.theme.userlist.player", {font = "Arial", size = 15, weight = 800})
surface.CreateFont("fchat.theme.prefix", 		{font = "Roboto", size = 16, weight = 400})
surface.CreateFont("fchat.theme.list.name", 	{font = "Arial", size = 14, weight = 400})
surface.CreateFont("fchat.theme.list.close", 	{font = "Arial", size = 12, weight = 800, antialias = false})
surface.CreateFont("fchat.theme.add", 			{font = "Tahoma", size = 18, weight = 800})
surface.CreateFont("fchat.theme.invite", 		{font = "Tahoma", size = 12, weight = 400})
surface.CreateFont("fchat.theme.row",			 {font = "Arial", size = 18, weight = 800})
...
function fchat.CreateFont(name, unique, font, size, weight, antialias)
...
fchat.CreateFont("Default", 				"fchat.theme.oss",	 	"Open Sans Semibold", 18, 400)
fchat.CreateFont("DejaVu Sans", 			"fchat.theme.text", 	"DejaVu Sans", 14, 580)
fchat.CreateFont("Tahoma", 					"fchat.tahoma", 		"Tahoma", 14, 1000)
fchat.CreateFont("Tiny Tahoma", 			"fchat.tahoma.tiny", 	"Tahoma", 11, 0)
fchat.CreateFont("Huge Tahoma", 			"fchat.tahoma.huge",	"Tahoma", 24, 0)
fchat.CreateFont("Arial", 					"fchat.arial", 			"Arial", 16, 0)
fchat.CreateFont("Coolvetica", 				"fchat.coolvetica", 	"Coolvetica", 20, 0)
fchat.CreateFont("Verdana", 				"fchat.verdana", 		"Verdana", 16, 0)
fchat.CreateFont("Akbar", 					"fchat.akbar", 			"Akbar", 22, 0)
fchat.CreateFont("Courier New", 			"fchat.courier.new", 	"Courier New", 16, 0)
fchat.CreateFont("Source Engine Chat Font", "fchat.verdana.se", 	"Verdana", 14, 700, false)

function fchat.GetFonts()
	return chatFonts
end
...
fchat.chat_x 				= fchat.config.New(nil, 								"chat_x", 			-1, 					true)
fchat.chat_y 				= fchat.config.New(nil, 								"chat_y", 			-1, 					true)
fchat.chat_w				= fchat.config.New(nil, 								"size_width", 		0, 						true)
fchat.chat_h 				= fchat.config.New(nil, 								"size_height", 		0, 						true)
fchat.font 					= fchat.config.New("Chatbox font", 						"font", 			"fchat.theme.oss", 	true)
fchat.chatSound				= fchat.config.New("Play chat sound", 					"chat_sound", 		true, 					true)
fchat.fadetime 				= fchat.config.New("Message fade out speed",			"fadetime", 		12, 					true)
fchat.timestamp 			= fchat.config.New("Enable timestamp", 					"timestamp", 		true, 					true)
fchat.snowFlakes 			= fchat.config.New("Snow flake amount", 				"snow", 			0, 						true)
fchat.enableAvatarsLocal 	= fchat.config.New("Enable avatars", 					"avatars_local", 	true, 					true)
fchat.smallAvatar 			= fchat.config.New("Use small avatars", 				"small_avatar", 	false, 					true)
fchat.maxHistory 			= fchat.config.New("Max chat history", 					"max_history", 		25, 					true)
fchat.extraShadow 			= fchat.config.New("Extra font shadow", 				"extra_shadow", 	false, 					true)
fchat.filterJoinDisconnect 	= fchat.config.New("Hide join/disconnect messages", 	"filter_jndc", 		false, 					true)
fchat.messageFadeIn 		= fchat.config.New("Chat message fade in", 				"fadein", 			true, 					true)

function fchat.font:OnChange(value)
	local invalid = fchat.FixInvalidFont()
	
	if (!invalid) then
		fchat.fontHeight = draw.GetFontHeight(value)
		
		fchat.BuildFontCache(" - ")
		fchat.BuildFontCache("ACCEPTED")
		fchat.BuildFontCache("-> ACCEPT <-")
	end
end
...
function fchat.snowFlakes:OnChange(value)
	value = tonumber(value)
	
	flakes = {}
	
	if (value > 0) then
		local w = fchat.chat_w:GetInt()
...
function theme:Initialize()
	local w, h, x, y = fchat.ScaleSize(200, true), fchat.ScaleSize(150), fchat.ScaleSize(10, true), fchat.ScaleSize(230)
	
	local panel = vgui.Create("fchat.chat")
	panel:SetAlpha(0)
	panel:SetSize(w, h)
	panel:SetPos(x, y)
	...
	if (fchat.chat_x:GetInt() == -1) 	then fchat.chat_x:SetInt(x) end
	if (fchat.chat_y:GetInt() == -1) 	then fchat.chat_y:SetInt(y) end
	if (fchat.chat_w:GetInt() == 0) 	then fchat.chat_w:SetInt(w) end
	if (fchat.chat_h:GetInt() == 0) 	then fchat.chat_h:SetInt(h) end

	panel.prefix = panel.bottom:Add("fchat.chat.prefix")
	panel.prefix:Dock(LEFT)
	
	panel.entry = panel.bottom:Add("fchat.chat.entry")
	panel.entry:Dock(FILL)
	panel.entry:DockMargin(2, 0, 0, 0)

	panel.list = panel:Add("fchat.chat.list")
	...
	panel.chatrooms = panel:Add("fchat.chatroom.container")
	...
	self.settingsIcon = panel:AddIcon("fchat/settings.png", function() fchat.theme.Call("ToggleConfigPanel") end, "Configuration")
	
	self.informationIcon = panel:AddIcon("fchat/emotes.png", function()
		if (!ValidPanel(self.expressionPanel)) then
			self.expressionPanel = vgui.Create("fchat.expressionlist")
	...
	local flakeAmount = fchat.snowFlakes:GetInt()
...
function theme:PaintGenericBackground(panel, w, h, text, x, y, xAlign, yAlign)
	...
	if (text) then
		draw.SimpleText(text, "fchat.theme.default.title", x or 6, y or 8, self.color.generic_label, xAlign or TEXT_ALIGN_LEFT, yAlign or TEXT_ALIGN_BOTTOM)
	end
end
...
function theme:PaintPrefix(w, h)
	...
	if (prefix) then
		draw.SimpleText(prefix, "fchat.theme.prefix", w /2, h /2, self.color.generic_label, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end
...
function theme:PaintChatroom(panel, w, h)
	...
	local active = fchat.theme.GetValue("current_chatroom")
	...
	if (name) then
		draw.SimpleText(name, "fchat.theme.list.name", w /2, h /2, self.color.generic_label, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	else
		draw.SimpleText("(" .. #panel.players .. ")", "fchat.theme.list.name", w -4, h /2, self.color.generic_label, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	end
end
...
local checkImage = Material("fchat/check.png", "smooth")

local function ParseConfig(sorted, list, admin)
	for i = 1, #sorted do
		local info = sorted[i]
		local object = fchat.config.Get(info.name)
		...
		if (info.index == 2) then
			...
			panel = vgui.Create("fchat.slider")
			panel:SetDecimals(0)
		...
		if (info.index == 3) then
			if (info.name == "theme") then
				local themes = fchat.theme.GetStored()
				...
				function panel:OnSelect(index, value, data)
					fchat.themeConfig:SetValue(data)
				end
			end
				...
				function panel:OnMousePressed()
					...
					for i = 1, #fonts do
						...
						function panel:Paint(w2, h2)
							...
							if (fchat.font:GetString() == data[2]) then
								draw.Material(w2 -24, h2 /2 -8, 16, 16, color_white, material)
							end
						end
						
						function panel.OnMousePressed()
							fchat.font:SetString(data[2])
...
local config = fchat.config.GetStored()
...
function theme:ToggleConfigPanel()
	if (!ValidPanel(self.config)) then
		self.config = vgui.Create("fchat.config")
		...
		if (LocalPlayer():IsSuperAdmin()) then
			...
			self.config:AddButton("Configure ranks", true, function()
				fchat.theme.Call("ToggleRankMenu")
			end)
			
			self.config:AddButton("Configure MySQL", true, function()
				self.mysqlPanel = vgui.Create("fchat.mysql")
			...
			self.config:AddButton("Configure Restrictions", true, function()
				self.restrictionPanel = vgui.Create("fchat.restrictions")
...
function theme:ToggleUserList()
	if (!ValidPanel(self.userListBase)) then
		...
		function self.userListBase:Paint(w, h)
			fchat.theme.Call("PaintGenericBackground", self, w, h, "User List")
		end
		...
		function self.userListBase:Rebuild(data, key)
			...
			if (IsValid(data.creator)) then
				...
				creator:SetFont("fchat.theme.userlist.player")
				creator:SizeToContents()
				creator:SetSkin("fchat")
			...
			for i = 1, #data do
				...
				if (IsValid(player) and player != data.creator) then
					...
					if (data.creator == LocalPlayer()) then
						...
						label:SetPos(18, 1)
						label:SetText(nick)
						label:SetFont("fchat.theme.userlist.player")
		...
		self.userListBase.list = self.userListBase:Add("fchat.chat.list")
		...
		self.userListBase.button = self.userListBase:Add("fchat.chat.button")
		self.userListBase.button:SetSize(75, 16)
		self.userListBase.button:SetText("Invite Player")
		self.userListBase.button:SetFont("fchat.theme.invite")
...
function theme:ToggleRankMenu()
	if (!ValidPanel(self.rankMenu)) then
		self.rankMenu = vgui.Create("fchat.ranks")
...
function theme:OnToggle(show)
	...
	if (show) then
		local x, y = fchat.chat_x:GetInt(), fchat.chat_y:GetInt()
		local w, h = fchat.chat_w:GetInt(), fchat.chat_h:GetInt()
		
		if (fchat.chat_x:GetInt() == -1) then x = fchat.ScaleSize(10, true)	fchat.chat_x:SetInt(x) end
		if (fchat.chat_y:GetInt() == -1) then y  = fchat.ScaleSize(230)	 	fchat.chat_y:SetInt(y) end
		if (fchat.chat_w:GetInt() == 0) then w = fchat.ScaleSize(200, true)	fchat.chat_w:SetInt(w) end
		if (fchat.chat_h:GetInt() == 0) then h = fchat.ScaleSize(150) 		fchat.chat_h:SetInt(h) end
...
local unpack = unpack
local GetType = type
local expressions = fchat.expression.GetStored()
	...
	local menu = DermaMenu()
		...
		if (LocalPlayer():IsSuperAdmin()) then
			...
			menu:AddOption("Set Title", function()
				...
				local editor = vgui.Create("fchat.editor")
	...
	while loop do
		for i = 1, #data do
			...
			if (type == "string" or type == "number") then
				value = fchat.BuildFontCache(value)
				...
				if (firstType) then
					...
					local canUse = title and true or hook.Run("FchatCanUseExpression", player, expression, firstType:GetUnique())
					local panelObject
					
					if (canUse) then
						panelObject = firstType:Execute(panel, unpack(found, 3))
					else
						panelObject = fchat.GenericLabel()
...
function theme:ParseData(data, list, isTitle)
	...
	for i = 1, #data do
		...
		if (type == "Player") then
			local avatarsEnabled = fchat.enableAvatars:GetBool()
			local avatarsEnabledLocal = fchat.enableAvatarsLocal:GetBool()
			local rankIconsEnabled = fchat.enableRankIcons:GetBool()
			local rankTitleEnabled = fchat.enableRankTitle:GetBool()
			
			local darkrpAllChat = false
			
			if (fchat.darkrpChat) then
				darkrpAllChat = fchat.darkrpChat:GetBool()
			end
			
			if (!DarkRP or (DarkRP and (data[2] == "(OOC) " or i == 1 or darkrpAllChat))) then
				if (avatarsEnabledLocal and avatarsEnabled) then
					local canUse = hook.Run("FchatCanUseExpression", value, "<avatar=" .. value:SteamID() .. ">", "avatar_steamid")
					...
					if (canUse) then
						local size = fchat.smallAvatar:GetBool() and 24 or 32
				...
				local wyoziteEnabled = fchat.enableWyoziteTags:GetBool()
				...
				if (rankTitleEnabled) then
					for userGroup, data in pairs(fchat.ranks) do
			...
			local label = fchat.GenericLabel()
			...
			function label:OnMouseReleased()
				if (self.wasPressed and CurTime() -self.wasPressed <= 0.16) then
					fchat.theme.Call("PlayerLabelPressed", self)
				end
				
				self.wasPressed = nil
			end
			
			fchat.BuildFontCache(text)
		...
		if (type == "Panel") then
			...
			if (value:GetClassName() == "Label") then
				...
				fchat.BuildFontCache(valueText, font)
...
function theme:ParseText(list, ...)
	...
	if (DarkRP and GetType(data[2]) == "string") then
		local player = util.FindPlayerFchat(data[2])
	...
	elseif (GAMEMODE.round_state) then
		...
		if (GetType(data[4]) == "string" and GetType(data[2]) == "string") then
			if (check) then
				local player = util.FindPlayerFchat(data[4])
				...
				if (#data > 6 and GetType(data[6]) == "string") then
					local player = util.FindPlayerFchat(data[6])
	...
	elseif (Core and Core.StyleName) then
		...
		for i = 1, #data do
			if (IsColor(data[i]) and data[i] == surf_color and isstring(data[i +1])) then
				local player = util.FindPlayerFchat(data[i +1])
	...
	if (IsValid(expressionPlayer)) then
		local canUse = hook.Run("FchatCanUseExpressions", expressionPlayer)
	...
	if (fchat.timestamp:GetBool()) then
	...
	local playSound = fchat.chatSound:GetBool()
