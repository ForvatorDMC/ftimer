...
function fchat.expression.New(text, unique)
	local expression = {}
...
function fchat.expression.GetStored()
	return stored
end
...
function fchat.expression.GetByExpression(expression)
	for i = 1, #stored do
...
function fchat.expression.GetByUnique(unique)
	for i = 1, #stored do
...
emoticons["garry"] = {"fchat/emoticons/garry.png", 64, 64}
emoticons["gaben"] = {"fchat/emoticons/gaben.png", 64, 64}
emoticons["228"] = {"fchat/emoticons/228.png", 64, 64}
emoticons["almaz"] = {"fchat/emoticons/almaz.png", 64, 64}
...
for match, data in pairs(emoticons) do
	local expression = fchat.expression.New(match, match)
...
local expression = fchat.expression.New("<noparse>(.-)</noparse>", "noparse")
...
local expression = fchat.expression.New("<url>(.-)</url>", "url")
...
local expression = fchat.expression.New("<c=(%d+,%d+,%d+)>(.-)</c>", "color")
...
local expression = fchat.expression.New("<avatar>", "avatar")
...
local expression = fchat.expression.New("<avatar=(STEAM_[0-5]:[01]:%d+)>", "avatar_steamid")
...
local expression = fchat.expression.New("<font=(.-)>(.-)</font>", "font")
...
local expression = fchat.expression.New("OverRustle", "overrustle")

expression.noPattern = true

function expression:Execute(base)
	local image = base:Add("DImage")
	image:SetImage("fchat/emoticons/overrustle.png")
	image:SetSize(32, 32)
...
local expression = fchat.expression.New("<lg>(.-)</lg>", "limegreen")
...
local expression = fchat.expression.New("<spoiler>(.-)</spoiler>", "spoiler")
...
local expression = fchat.expression.New("<hsv>(.-)</hsv>", "hsv")
...
local expression = fchat.expression.New("<flash%s*(c?=?%d-,-%d-,-%d-)>(.-)</flash>", "flash")
...
local expression = fchat.expression.New("<vscan%s*(c?=?%d-,-%d-,-%d-)>(.-)</vscan>", "vscan")
...
local expression = fchat.expression.New("<hscan%s*(c?=?%d-,-%d-,-%d-)>(.-)</hscan>", "hscan")
...
local expression = fchat.expression.New("<reverse>(.-)</reverse>", "reverse")
...
local expression = fchat.expression.New("<cflash%s*(c?=?%d-,-%d-,-%d-)%s*(c?=?%d-,-%d-,-%d-)>(.-)</cflash>", "cflash")
...
local expression = fchat.expression.New("<icon%s?(%d+),-(%d+)>(.-)</icon>", "icon")
...
local expression = fchat.expression.New("<semote>(.-)</semote>", "semote")
